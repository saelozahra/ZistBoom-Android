...
Version=7.3
@EndOfDesignText@
'xCustomListViewMod v1.65 - modified version to support expandable items
...
Sub Class_Globals
	...
	Public AnimationDuration As Int = 313
...
Private Sub InsertAtImpl(Index As Int, Pnl As B4XView, ItemSize As Int, Value As Object, InitialSize As Int)
	...
	Else
		...
		Else
			...
			'***********************************
			Dim ShouldSetPanelHeight As Boolean = InitialSize > ItemSize
			If ShouldSetPanelHeight = True Then 'collapse
				Pnl.Height = InitialSize
			Else
				p.SetLayoutAnimated(0, 0, offset, p.Width, ItemSize)
			End If
			p.SendToBack
			'****************************************************************
